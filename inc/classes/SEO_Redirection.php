...
if (!class_exists('CFGP_SEO_Redirection', false)) : class CFGP_SEO_Redirection extends CFGP_Global
{
	...
	private function do_redirection($redirect){
		...
		if( 
			!empty($redirect['country'])
			&& empty($redirect['region'])
			&& empty($redirect['postcode'])
			&& !empty($redirect['city']) 
		) {
			$mode = 'country_city';
		}
